...
public class OficinaAppService { 
    ...
    public OficinaAppService(OficinaRepository repository) { _repository = repository; }
    public async Task<OficinaResponseDto> CriarAsync(CriarOficinaDto dto, int usuarioId) 
    { var oficina = new Oficina(usuarioId, dto.Nome, dto.Telefone, dto.Endereco, dto.Logotipo); 
        var oficinaCriada = await _repository.AdicionarAsync(oficina); return MapearResponse(oficinaCriada); }
    
    public async Task<OficinaResponseDto?> ObterPorIdAsync(Guid id) { 
        var oficina = await _repository.ObterPorIdAsync(id); if (oficina == null) 
            return null; 
    ...
    public async Task<List<OficinaResponseDto>> ListarAsync(){ 
        ...
        return oficinas.Select(MapearResponse)
            .ToList(); 
    }
